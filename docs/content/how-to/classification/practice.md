...
from tirex.models.classification import TirexRFClassifier, TirexClassifierTorch
...
For Linear classifier, you have the possibility to provide a validation set.

```python
classifier.fit(train_data=(train_X, train_y),val_data=(val_X, val_y))
```

If you don't provide it, the train dataset will be split into train and validation datasets internally. Two parameters are responsible for the train/validation split by model initialization:
- `val_split_ratio` (float): is responsible for the size of the validation set (between 0 and 1). Defaults to: 0.2
...
# Convert to numpy if needed (predict returns torch.Tensor)
